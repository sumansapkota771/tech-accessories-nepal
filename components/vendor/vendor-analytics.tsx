...
const revenueConfig: ChartConfig = {
  revenue: { label: "Net revenue (Rs.)", color: "var(--chart-2)" },
}
...
const amountConfig: ChartConfig = {
  amount: { label: "Revenue (Rs.)", color: "var(--chart-2)" },
}
  ...
  useEffect(() => {
    async function fetchAnalytics() {
      try {
        ...
        const [suborderRes, itemsRes] = await Promise.all([
          supabase
            .from("suborders")
            .select("created_at, subtotal, commission_amount, status")
            .eq("vendor_id", vendorId)
            .gte("created_at", thirtyDaysAgo.toISOString()),
          supabase.from("order_items").select("price, quantity, products ( name )").eq("vendor_id", vendorId),
        ])

        // Net revenue over the last 30 days, one bucket per day
        ...
        for (const suborder of suborderRes.data || []) {
          const key = suborder.created_at?.slice(0, 10)
          if (key && byDay.has(key)) {
            const net = (suborder.subtotal || 0) - (suborder.commission_amount || 0)
        ...
        for (const suborder of suborderRes.data || []) {
          if (statusCounts.has(suborder.status)) {
            statusCounts.set(suborder.status, (statusCounts.get(suborder.status) || 0) + 1)
          }
        }
        setStatusSeries(
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
      <Card className="md:col-span-2">
        <CardHeader>
          <CardTitle>Net revenue, last 30 days</CardTitle>
          <CardDescription>Rs. {totalRevenue30d.toLocaleString()} after commission</CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <CardContent>
          {topProducts.length === 0 ? (
            ...
            <ChartContainer config={amountConfig} className="h-64 w-full">
              <BarChart data={topProducts} layout="vertical" margin={{ left: 16, right: 48 }}>
